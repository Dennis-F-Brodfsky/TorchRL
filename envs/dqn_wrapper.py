...
class Monitor(Wrapper):
        ...
        if force:
            ...
            if len(training_manifests) > 0:
                raise error.Error(
                    ...
                    You should use a unique directory for each training run, 
                    or use 'force=True' to automatically clear previous monitor files.'''.format(
    ...
    def _set_mode(self, mode):
        ...
        self.stats_recorder.type = ele_type
    ...
    def _env_info(self):
        env_info = {'gym_version': version.VERSION,}
        if self.env.spec:
...
def collapse_env_infos(env_infos, training_dir):
    ...
    for other in env_infos[1:]:
        if first != other:
            raise error.Error(
                'Found two unequal env_infos: {} and {}. '
                'This usually indicates that your training directory {} has commingled '
                'results from multiple runs.'.format(
